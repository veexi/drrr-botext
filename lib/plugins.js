/* plugin hook */
room_hook_plugin = `var [event_me, event_music, event_leave, event_join, event_newhost, event_msg, event_dm, event_dmto, event_newtab, event_exittab, event_exitalarm, event_logout, event_musicbeg, event_musicend, event_timer, event_clock, event_kick, event_ban, event_unban, event_roll, event_roomprofile, event_roomdesc, event_timeout, event_lounge, event_low] = ["me", "music", "leave", "join", "new-host", "msg", "dm", "dmto", "newtab", "exittab", "exitalarm", "logout", "musicbeg", "musicend", "timer", "clock", "kick", "ban", "unban", "roll", "room-profile", "new-description", "timeout", "lounge", "low"];
event_events = [event_me      , event_music   , event_leave   , event_join    , event_newhost , event_msg     , event_dm      , event_dmto    ,/* event_logout  , */event_musicbeg , event_musicend, /*event_timer, event_clock, */event_kick, event_ban, event_unban, event_roll, event_roomprofile, event_roomdesc, event_lounge, event_newtab, event_low, "*"]
...
function MsgDOM2EventObj(msg, info){
  ...
  try{
    ...
    if(msg.classList.contains("system")){
      ...
      let $name = $(msg).find('.name');
      let $user = $name.find('span');
...
function roomProfile(){
  ...
  if (typeof window !== 'undefined' && window.extension_profile) {
    icon = window.extension_profile.icon || "";
    id = window.extension_profile.id || "";
    name = window.extension_profile.name || "";
    tripcode = window.extension_profile.tripcode || "";
  }
  else if (typeof window !== 'undefined' && window.profile) {
    icon = window.profile.icon || "";
    id = window.profile.id || "";
    name = window.profile.name || "";
    tripcode = window.profile.tripcode || "";
  }
  else if (typeof window !== 'undefined' && window.user) {
    icon = window.user.icon || "";
    id = window.user.id || "";
    name = window.user.name || "";
    tripcode = window.user.tripcode || "";
  }
...
function drrr_send(msg, url, to){
  ...
  if(to){
    ...
    ctrlRoom(cmd);
    if (typeof draw_message === 'function') {
      draw_message(String(msg), undefined);
    }
  }
}
...
var drrr = {
  ...
  'low': function(msg, url){
    cmd = {"message": String(msg), "loudness": 3};
    if(url) cmd['url'] = url;
    ctrlRoom(cmd);
    if (typeof draw_message === 'function') {
      draw_message(String(msg), undefined, true);
    }
  },
  'chown': function(user){
    findUser(user, (u)=>{
