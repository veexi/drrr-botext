...
let event_events = [event_me      , event_music   , event_leave   , event_join    , event_newhost , event_msg     , event_dm      , event_dmto    ,/* event_logout  , */event_musicbeg , event_musicend, /*event_timer, event_clock, */event_kick, event_ban, event_unban, event_roll, event_roomprofile, event_roomdesc, event_lounge, event_newtab, event_low, "*"]
...
let lowvoice_message = "lowvoice_message"
      ...
      if(config['profile'] && !flush
        ...
        success: function(data){
          ...
          if(typeof document === 'undefined'){
            chrome.cookies.getAll({url: 'https://drrr.com'}, (cookies) => {
              cookies = cookies.filter(cookie => cookie.name === 'drrr-session-1');
              chrome.storage.sync.set({profile: Profile, cookie: cookies});
            });
          } else {
