...
function ne_fetch(cb, id){
  fetch(`https://[NO MAINTAINED].com/song/url?id=${id}`)
    .then(res => res.json())
    .then(data => cb(data.data[0].url))
    .catch(err => notification_alert("fetch failed"));
}
...
function yt_fetch(cb, url){
  var callback = (fullURL) => {
    fetch(fullURL)
      .then(res => res.json())
      .then(data => cb(yt_songs(data)[0]['link']))
      .catch(err => notification_alert("fetch failed" + JSON.stringify(err)));
  };
  chrome.storage.sync.get("youtube-api-url", (config)=>{
    if(config["youtube-api-url"])
...
function tenor_img(word, callback){
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      let beg = Math.floor(Math.random()*limit);
      for(let i = 0; i < limit; i++){
        let idx = (beg + i) % limit;
        for(let type of ['gif', 'nanogif', 'tinygif', 'mediumgif']){
    ...
    .catch(err => callback('http://tomchun.tw/tomchun/wp-content/uploads/2018/10/4536.jpg'));
}
...
function giphy_img(word, callback){
  ...
  fetch(url)
    ...
    .then(res => {
      ...
      let beg = Math.floor(Math.random()*limit);
      if(!word.length){
        if(data.images && data.images.original && data.images.original.url)
          return callback(data.images.original.url);
        return callback('https://thumbs.dreamstime.com/b/no-found-symbol-unsuccessful-search-suitable-results-oops-page-failure-concept-flat-outline-vector-illustration-loupe-122872462.jpg');
      }
      for(let i = 0; i < limit; i++){
        let idx = (beg + i) % limit;
        for(let type of ['gif', 'nanogif', 'tinygif', 'mediumgif']){
